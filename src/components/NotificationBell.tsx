...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((o) => !o)} className="relative rounded-lg p-2 text-white/80 hover:bg-white/10" aria-label={t.notifications}>
        ...
        {unread > 0 && <span className="absolute -top-0.5 -end-0.5 min-w-[18px] rounded-full bg-umber-500 px-1 text-center text-[10px] font-bold text-white">{unread}</span>}
      </button>
      {open && (
        <div className="absolute end-0 z-50 mt-1 w-80 max-h-96 overflow-auto rounded-xl bg-white text-gray-800 shadow-xl ring-1 ring-black/10">
          <div className="flex items-center justify-between border-b px-3 py-2 text-sm">
            <span className="font-semibold">{t.notifications}</span>
            {unread > 0 && <button onClick={markAll} className="text-xs text-forest-400">{t.markAllRead}</button>}
          </div>
          {items.length === 0 ? <p className="p-4 text-center text-sm text-gray-500">{t.noNotifications}</p> : (
            <ul className="divide-y">
              {items.map((n) => (
                <li key={n.id} className={`px-3 py-2 text-sm ${n.is_read ? "" : "bg-wheat-100"}`}>
