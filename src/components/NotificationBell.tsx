...
export default function NotificationBell({ variant = "dark" }: { variant?: "dark" | "pill" }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((o) => !o)} className={variant === "pill" ? "pill-icon relative" : "relative rounded-xl p-2 text-white/80 hover:bg-white/10"} aria-label={t.notifications}>
      ...
      {open && (
        <div className="absolute end-0 z-50 mt-1 w-80 max-h-96 overflow-auto rounded-2xl bg-white text-ink shadow-xl ring-1 ring-black/5">
          ...
          {items.length === 0 ? <p className="p-4 text-center text-sm text-muted">{t.noNotifications}</p> : (
              ...
              {items.map((n) => (
                <li key={n.id} className={`px-3 py-2 text-sm ${n.is_read ? "" : "bg-surface"}`}>
                  <Link href={n.link ?? "/"} onClick={() => { markOne(n.id); setOpen(false); }} className="block">
                    ...
                    {n.body && <div className="line-clamp-2 text-xs text-muted">{n.body}</div>}
                    <div className="text-[11px] text-muted">{new Date(n.created_at).toLocaleString()}</div>
                  </Link>
                </li>
              ))}
            </ul>
